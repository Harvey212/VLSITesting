...
ATPG::wptr ATPG::find_pi_assignment(const wptr object_wire, const int &object_level) {
  ...
  if (object_wire->is_input()) {
    ...
    */	
    ...
    //int garbage=0;
    const nptr& node1 =object_wire->inode.front();
    ...
    switch (node1->type)
    {
      ...
      case NAND:
          if (object_level)
          {
            new_object_wire = find_easiest_control(node1);
          }else
          {
            new_object_wire = find_hardest_control(node1);
          }
        break;
      case AND:
      case NOR:
      ...
      case BUF:
      case NOT:
        new_object_wire = node1->iwire.front();
        break;
    }
...
bool ATPG::trace_unknown_path(const wptr w) {
  ...
  {
    int size =w->onode.size();

    for(int i=0;i<size;i++)
    {
      const wptr& wire1 = w->onode[i]->owire.front();
      if((wire1->value==U))
      {
          if(trace_unknown_path(wire1))
          {
            return true;
          }
      }
